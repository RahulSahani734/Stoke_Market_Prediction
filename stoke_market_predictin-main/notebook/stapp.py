...
if uploaded_file is not None:
    ...
    if close_col is None:
        ...
        n_steps = 100
        x_input = scaled_close[-n_steps:].reshape(1, -1)
        temp_input = list(x_input[0])

        # Forecast for future_days
        lst_output = []
        n_steps = 100
        # Take the last n_steps from the data as the starting point
        ...
        while i < future_days:
            ...
            # Append the prediction and slide the window for the next prediction
        ...
        predicted_future = scaler.inverse_transform(lst_output)
        predicted_future = scaler.inverse_transform(np.array(lst_output).reshape(-1, 1))

        # Build day indices
        day_new = np.arange(len(close_data))
        day_pred = np.arange(len(close_data), len(close_data) + future_days)

        # Plot
        st.subheader("Future Forecast (Next {} Days)".format(future_days))
